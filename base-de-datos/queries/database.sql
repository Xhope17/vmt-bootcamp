...
-- 1. Declarar variables para mantener los mismos UUIDs en las relaciones
DECLARE @IdCollaborator UNIQUEIDENTIFIER = NEWID();

DECLARE @IdTag1 UNIQUEIDENTIFIER = NEWID();
DECLARE @IdTag2 UNIQUEIDENTIFIER = NEWID();
DECLARE @IdTag3 UNIQUEIDENTIFIER = NEWID();
DECLARE @IdTag4 UNIQUEIDENTIFIER = NEWID();

DECLARE @IdPost1 UNIQUEIDENTIFIER = NEWID();
DECLARE @IdPost2 UNIQUEIDENTIFIER = NEWID();

-- 2. Insertar un Colaborador de prueba (Autor de los posts)
INSERT INTO Collaborators (Id, FullName, Position)
VALUES 
    (@IdCollaborator, 'Ana Backend', 'Senior Software Engineer');

-- 3. Insertar Tags de prueba
INSERT INTO Tags (TagId, Name)
VALUES 
    (@IdTag1, 'SQL Server'),
    (@IdTag2, 'C#'),
    (@IdTag3, 'Buenas Prácticas'),
    (@IdTag4, 'Rendimiento');

-- 4. Insertar Posts de prueba
INSERT INTO Posts (Id, CollaboratorId, Title, Content)
VALUES 
    (@IdPost1, @IdCollaborator, 'Optimizando consultas con Índices', 'En este artículo veremos cómo los índices clustered y non-clustered mejoran...'),
    (@IdPost2, @IdCollaborator, 'Arquitectura Limpia en .NET', 'Implementar Clean Architecture en proyectos de C# nos permite...');

-- 5. Relacionar Posts y Tags (PostTags)
-- Post 1 se etiqueta con: SQL Server, Rendimiento, Buenas Prácticas
-- Post 2 se etiqueta con: C#, Buenas Prácticas
INSERT INTO PostTags (PostId, TagId)
VALUES 
    (@IdPost1, @IdTag1), -- Post 1 -> SQL Server
    (@IdPost1, @IdTag4), -- Post 1 -> Rendimiento
    (@IdPost1, @IdTag3), -- Post 1 -> Buenas Prácticas
    (@IdPost2, @IdTag2), -- Post 2 -> C#
    (@IdPost2, @IdTag3); -- Post 2 -> Buenas Prácticas
GO